...
def process_case(
    ...
    with tracer.start_as_current_span(f"process_case_{str(case.case_id)}"):
        ...
        variants_table_name = f"{namespace}.somatic_snv_variant"
        consequences_table_name = f"{namespace}.somatic_snv_consequence"
        if not vcf.samples:
            ...
            for record in vcf:
                if len(record.ALT) <= 1:
                    if record.CHROM in SUPPORTED_CHROMOSOMES:
                        ...
                        occurrences = process_occurrence(record, common=common)
