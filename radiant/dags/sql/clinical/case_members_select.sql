...
-- back as silence.
--
-- The doubled percent sign in the LIKE below is not a typo. This statement carries bound
-- parameters and RadiantStarRocksOperator ends at `cursor.execute(sql, parameters)`, so
-- every literal percent sign has to be doubled or the driver reads it as a placeholder --
-- including any inside a comment, which is how this line came to be worded this way.
WITH gvcf_doc AS (
...
-- Whatever this resolves to, resolve_cases asserts gvcf_matches = 1 per member -- that
-- assertion, not this rule, is the actual safeguard.
