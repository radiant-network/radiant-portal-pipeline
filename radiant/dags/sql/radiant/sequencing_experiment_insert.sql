INSERT INTO {{ params.starrocks_staging_sequencing_experiment }} (
            ...
            request_id, request_priority, vcf_filepath, exomiser_filepath, sex, family_role, affected_status,
            created_at, updated_at, ingested_at
) VALUES (
    %(case_id)s, %(seq_id)s, %(task_id)s, %(part)s, %(analysis_type)s, %(aliquot)s, %(patient_id)s,
    %(experimental_strategy)s, %(request_id)s, %(request_priority)s, %(vcf_filepath)s, %(exomiser_filepath)s, %(sex)s,
