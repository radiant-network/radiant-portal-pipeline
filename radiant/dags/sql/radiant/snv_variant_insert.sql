...
SELECT
    v.locus_id,
    COALESCE(gf.pc_wgs, 0) / NULLIF((SELECT pn_wgs FROM germline_pn), 0)       AS germline_pf_wgs,
    COALESCE(gf.pc_wxs, 0) / NULLIF((SELECT pn_wxs FROM germline_pn), 0)       AS germline_pf_wxs,
    COALESCE(sf.pc_tn_wgs, 0) / NULLIF((SELECT pn_tn_wgs FROM somatic_pn), 0)  AS somatic_pf_tn_wgs,
    COALESCE(sf.pc_tn_wxs, 0) / NULLIF((SELECT pn_tn_wxs FROM somatic_pn), 0)  AS somatic_pf_tn_wxs,
    v.gnomad_v3_af,
    v.topmed_af,
    v.tg_af,
    ...
    COALESCE(gf.pc_wgs_affected, 0) / NULLIF((SELECT pn_wgs_affected FROM germline_pn), 0)  AS germline_pf_wgs_affected,
    ...
    COALESCE(gf.pc_wgs_not_affected, 0) / NULLIF((SELECT pn_wgs_not_affected FROM germline_pn), 0) AS germline_pf_wgs_not_affected,
    ...
    COALESCE(gf.pc_wxs_affected, 0) / NULLIF((SELECT pn_wxs_affected FROM germline_pn), 0)  AS germline_pf_wxs_affected,
    ...
    COALESCE(gf.pc_wxs_not_affected, 0) / NULLIF((SELECT pn_wxs_not_affected FROM germline_pn), 0) AS germline_pf_wxs_not_affected,
