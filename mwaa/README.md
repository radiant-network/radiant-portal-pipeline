...
| `RADIANT_OPEN_DATA_USE_LEGACY_TABLES`   | Sources held back on their pre-contract Radiant Iceberg tables instead of read from OpenDataLake. **Empty by default** -- every contract table comes from OpenDataLake, which is the target state. Listing a source is an exception to it: for a partial environment that cannot hold all 15 contract tables, or one whose publish is not ready. Accepts either name a source answers to (`mondo` or `mondo_term`). An unknown name is rejected rather than silently ignored. |
