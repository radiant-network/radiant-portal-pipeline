...
- `import_open_data.py`, `import_brim.py` — additional sources. `import_open_data` stays `schedule=None`: it is the reference load, triggered as P1 by `reannotate_open_data`, and its two file-driven broker loads (ClinVar RCV summary, cytoband) skip unless the caller passes their paths.
- `reannotate_open_data.py` — the weekly (Saturday 00:00) OpenDataLake refresh + re-annotation, P1→P4. Holds the same `import_mutex` S3 lock as `import_part` for the whole run, which is why the reference load and the re-annotation are one DAG rather than two. Rebuilds `snv__staging_variant`/`snv__consequence` by upsert, then the four portal-facing SNV tables and the CNV occurrences, then records the release in `open_data_release`. Tenants and parts are discovered from `staging_sequencing_experiment` at run time (design: `design/SJRA-1811-opendatalake-integration.md` §4–§5).
