...
RUN apt-get update && apt-get install -y curl bzip2 build-essential zlib1g-dev libbz2-dev liblzma-dev libcurl4-openssl-dev libssl-dev && apt-get clean
...
RUN pip install apache-airflow-providers-postgres apache-airflow-providers-celery==3.10.0 pyiceberg[s3fs] pyarrow
