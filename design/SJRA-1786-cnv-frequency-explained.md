...
```
furthest a partner can start  =  0.25 x 2 047  ~=  512
```

Now measure that 512 against the grid of each bucket the probe visits:

| target bucket | grid width | max drift | drift as a fraction of one cell |
|---|---|---|---|
| 11 | 2 048 | 512 | 0.25 |
| 10 | 1 024 | 512 | 0.50 |
| **9** | **512** | **512** | **1.00** ← the tight one |

In every case the drift is **less than one cell of the grid being searched**. And if two points sit less
than one cell apart, their cell numbers differ by at most one. So the correct cell is either the one the
query lands in or an immediate neighbour — **`pos_bin ±1` is enough.**

Note how little room is left in the bottom row. At 80% reciprocal overlap the `b − 1` case lands exactly on
the limit. There is no margin, which is what §5.11 is about.
