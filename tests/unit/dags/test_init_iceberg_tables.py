...
def test_dag_has_correct_number_of_tasks(dag_bag):
    dag = dag_bag.get_dag(f"{NAMESPACE}-init-iceberg-tables")
    assert len(dag.tasks) == 7  # 1 get namespace + 1 init + 4 germline tables
...
def test_dag_has_correct_task_dependencies(dag_bag):
    ...
    assert (
        "create_variant_table_k8s"
        in dag.get_task("create_consequence_table_k8s").upstream_task_ids
    )
    assert (
        "create_consequence_table_k8s" in dag.get_task("create_germline_snv_occurrence_table_k8s").upstream_task_ids
    )
    assert (
