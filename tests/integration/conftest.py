...
@pytest.fixture(scope="session")
def sample_cosmic_gene_set_tsv(s3_fs):
    """
    Uploads the sample COSMIC Cancer Gene Census TSV (gzipped, header row included) to S3.
    """
    src_path = RESOURCES_DIR / "open_data" / "cosmic" / "Cosmic_CancerGeneCensus_GRCh38.tsv.gz"
    dest_path = "opendata/cosmic/"
    s3_fs.put(src_path, dest_path)
    yield f"{dest_path}Cosmic_CancerGeneCensus_GRCh38.tsv.gz"
